...
A global function that determines whether the form should operate in create mode or edit mode. When the form component mounts, `<VueForm>` calls `isNewItemCheck` with the `context`. Within context, you can find `itemId` prop and it's value which can be useful to identify the mode.
